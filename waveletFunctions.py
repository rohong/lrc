...
# # CWT  1D Wavelet transform with optional significance testing
...
def cwt(Y, dt, pad=0, dj=-1, s0=-1, J1=-1, mother=-1, param=-1):
    n1 = len(Y)
    ...
    if pad == 1:
        ...
        x = np.concatenate(
            (x, np.zeros((2 ** (base2 + 1) - n1).astype(np.int64))))
    ...
    wave = np.zeros(shape=(int(J1 + 1), n), dtype=complex)

    # loop through all scales and compute transform
    # for a1 in range(0, int(J1+1)):
    for a1, scale_a1 in enumerate(scale):
        daughter, fourier_factor, coi, dofmin = wave_bases(
            mother, k, scale_a1, param)
        wave[a1, :] = np.fft.ifft(f * daughter)  # wavelet transform[Eqn(4)]

    # for negative scales
    if np.iscomplex(x).any():
        fneg = np.fft.fft(np.conj(x))
        waveneg = np.empty_like(wave)
        # for a1 in range(0, int(J1+1)):
        for a1, scale_a1 in enumerate(scale):
            daughter, fourier_factor, coi, dofmin = wave_bases(
                mother, k, scale_a1, param)
            # wavelet transform[Eqn(4)]
            waveneg[a1, :] = np.conj(np.fft.ifft(fneg * daughter))
        waveneg = waveneg[:, :n1]
    ...
    if np.iscomplex(x).any():
        wave = np.concatenate((wave[::-1, :], waveneg), axis=0)  # reverse order
        period = np.hstack((period[::-1], -period))  # reverse order to make frequency order same as spectrogram
        scale = np.hstack((scale[::-1], -scale))  # reverse order

        # wave = np.concatenate((wave, waveneg[::-1, :]), axis=0)
        # period = np.hstack((period, -period[::-1]))
        # scale = np.hstack((scale, -scale[::-1]))
...
def wave_signif(Y, dt, scale, sigtest=0, lag1=0.0, siglvl=0.95,
    ...
    if sigtest == 0:  # no smoothing, DOF=dofmin [Sec.4]
        ...
        dof[dof < 1] = 1
        dof = dofmin * \
              np.sqrt(1 + (dof * dt / gamma_fac / scale) ** 2)  # [Eqn(23)]
        ...
        if navg == 0:
            print('ERROR: No valid scales between ' +
                  str(s1) + ' and ' + str(s2))
