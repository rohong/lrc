...
def compute_Hc(series, n=10, kind="random_walk", min_window=10,
               max_window=None, simplified=True):
    """
    ...
    window_sizes = list(map(
        lambda x: int(n ** x),
        np.arange(math.log10(min_window) / math.log10(n),
                  math.log10(max_window) / math.log10(n), 0.2)))
    ...
    if not cumprod:  # ordinary increments
        ...
        for i in range(1, length):
            if i < min_lookback + 1:
                ...
                direction = np.sign(
                    series[i - 1] - series[i - 1 - lookback]) * np.sign(
        ...
        for i in range(1, length):
            if i < min_lookback + 1:
                ...
                lookback = np.random.randint(min_lookback,
                                             min(i - 1, max_lookback) + 1)
                direction = np.sign(
                    series[i - 1] / series[i - 1 - lookback] - 1.) * np.sign(
                    proba - np.random.uniform())
            series[i] = series[i - 1] * np.fabs(
                1 + np.random.randn() / 1000. * direction)
...
if __name__ == '__main__':
    ...
    series = np.cumprod(
        random_changes)  # create a random walk from random changes
